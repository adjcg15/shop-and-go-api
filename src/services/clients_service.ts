...
async function createPaymentMethodToClient(
    ...
    try {
        ...
        for (const paymentMethod of existingPaymentMethods) {
            const paymentMethodsAreSame = await compareHashedString(
                cardNumber,
                paymentMethod.hashedCardNumber
            );
        ...
        const { encryptedCardNumber, initialVector, authenticationTag } =
...
async function deletePaymentMethodFromClient(
    idClient: number,
...
async function createClient(client: {
    ...
    try {
        const { password, birthdate, fullName, phoneNumber } = client;
...
async function createAddressToClient(
    ...
    try {
        ...
        if (existingAddress !== null) {
            throw new BusinessLogicException(
                ErrorMessages.ADDRESS_ALREADY_EXISTS,
